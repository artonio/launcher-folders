...
class MainWindow(Gtk.Window):
	...
	def saveDrawerClicked(self, widget):
		...
		if currentPage == -1:
			...
			#if len(ourIconView.settingsList) > 0:
			if len(ourIconView.drawerSettings['appList']) > 0:
				...
				if ourScrolledWindow.isNewFile:
					...
					subprocess.call([CURR_WORK_DIR + '/deleteAddIconLauncher.sh', str(ourScrolledWindow.drawerName)])
				else:
					if os.path.isfile(CONFIG_DIR + ourScrolledWindow.drawerName + ".png"):
						...
						ourIconView.drawerSettings['drawerIcon'] = ourScrolledWindow.drawerIconFileName
					subprocess.call([CURR_WORK_DIR + '/deleteAddIconLauncher.sh', str(ourScrolledWindow.drawerName)])
					print ourScrolledWindow.drawerName
...
class Tabs(Gtk.Notebook):
	...
	def createPage(self, drawerLabel, drawerIconFileName, isNewFile):
		...
		tab_label.connect("edit-drawer-name", self.on_edit_drawerName, self, scrolled_window)

		self.append_page(scrolled_window, tab_label)
		self.show_all()

	#When editing an existing drawer, will create a new drawer instead of renaming it
	def on_edit_drawerName(self, tab_label, notebook, tab_widget):
		print "edit drawer name"
		editDrawerNameDialog = EditDrawerNameDialog(self.parent)
		editDrawerNameDialog.drawerName.set_text(tab_label.label.get_text())
		response = editDrawerNameDialog.run()
		if response == Gtk.ResponseType.OK:
			newDrawerName = editDrawerNameDialog.drawerName.get_text()
			tab_label.label.set_text(newDrawerName)
			currentPage = notebook.get_current_page()
			ourScrolledWindow = notebook.get_nth_page(currentPage)
			ourIconView = ourScrolledWindow.getIconView()
			ourScrolledWindow.drawerName = newDrawerName
			ourIconView.drawerSettings['drawerName'] = newDrawerName
		elif response == Gtk.ResponseType.CANCEL:
			print "cancel"
		editDrawerNameDialog.destroy()
...
class EditDrawerNameDialog(Gtk.Dialog):
	def __init__(self, parent):
		Gtk.Dialog.__init__(self, "Drawer Name", parent, 0, 
			(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL, Gtk.STOCK_OK, Gtk.ResponseType.OK))
		box = self.get_content_area()
		self.drawerName = Gtk.Entry()
		hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
...
class ScrolledWindowIconView(Gtk.ScrolledWindow):
	def __init__(self, targets, drawerName, drawerIconFileName, isNewFile):
		...
		if not self.isNewFile:
			...
			#configList = list(util.getConfigFromFile(configFile))
			...
			for uri in uris:
				...
				if matchUrl:
					...
					if fileExtension == ".desktop":
						...
						file_name = urllib.unquote(uri)
						print "file_name: " + file_name
						#trim uri to file name format
						file_name = file_name.replace("file://", "")
						#get mimetype from file name
						type_ = magic.from_file(file_name, mime=True)
						print type_
						#get icon
						if type_:
						...
						fileName = gio_file.get_basename()
						fileIcon = info.get_filename()
						#fileIcon = self.lookupIcon(uri_icon)
						print fileName, fileIcon
						#set UI model 
						row = self.model.append()
						self.model.set_value(row, COLUMN_TEXT, fileName)
						self.model.set_value(row, COLUMN_PIXBUF, self.getPixBuffFromFile(fileIcon, self.iconSize))
						#update drawer settings dictionary
